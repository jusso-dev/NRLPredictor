...
Laravel 11 + Livewire 3 app that predicts NRL try scorers and match winners using scraped data from nrl.com and an optional Claude AI refinement pass. Runs in Docker Compose with 5 services (app, queue, scheduler, agent, mysql).
...
├── agent.py               # Claude Agent SDK loops (analysis + chat)
...
Key env vars for AI features: `CLAUDE_AGENT_TOKEN`, `CLAUDE_AGENT_INTERNAL_SECRET`. Without these, AI review and chat are disabled but statistical predictions still work. `ODDS_API_KEY` enables bookmaker odds integration.
...
- **Chat not responding**: Ensure `CLAUDE_AGENT_TOKEN` and `CLAUDE_AGENT_INTERNAL_SECRET` are set, and the agent container is running.
